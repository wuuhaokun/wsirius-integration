 ...
 */
@Getter
public enum WStatus implements IWStatus {
    /**
     * 操作成功！
     */
    SUCCESS(200, "操作成功！"),
    ...
    WStatus(Integer code, String message) {
        this.code = code;
        this.message = message;
    }

    public static WStatus fromCode(Integer code) {
        WStatus[] statuses = WStatus.values();
        for (WStatus status : statuses) {
            if (status.getCode()
                    .equals(code)) {
                return status;
            }
